...
export class Viewer {
    ...
    constructor(canvas, settings, stats, width, height) {
        ...
        this.sectionPlaneValuesDisabled = new Float32Array(4);//剖面
        ...
        this.pickIdToViewObject = [];//存储pickId，用来获取viewObject
    ...
    init() {
        var promise = new Promise((resolve, reject) => {
            ...
            this.programManager = new ProgramManager(this.gl, this.settings);//根据设置创建着色器,缓冲区地址管理等
            ...
            this.oitBuffer = new RenderBuffer(this.canvas, this.gl, COLOR_ALPHA_DEPTH);//透明度
            this.quad = new SSQuad(this.gl);
            this.quad2 = new WSQuad(this, this.gl);//剖面
        });
        return promise;
    }
    ...
    render(now) {
        ...
        if (this.dirty == 2 || (this.dirty == 1 && now - this.lastRepaint > 500)) {
            ...
            this.dirty = 0;
            //console.info("viewer473----"+this.invisibleElements.nonce);
...
debugger;
        ...
        gl.clearDepth(1);
        gl.clearStencil(0);
        gl.enable(gl.DEPTH_TEST);//启用深度缓冲Z-Buffer，检查像素深度，
        gl.depthFunc(gl.LEQUAL);

        gl.viewport(0, 0, this.width, this.height);
        gl.clear(gl.COLOR_BUFFER_BIT | gl.DEPTH_BUFFER_BIT | gl.STENCIL_BUFFER_BIT);// 清空画布和深度缓冲
        gl.disable(gl.CULL_FACE);//关闭剔除背面三角形
        ...
        gl.enable(gl.CULL_FACE);//开启剔除背面三角形
    ...
    /**
     ...
     @param {*} params
     @param {Array} {params.canvasPos,params.uniqueId} Canvas coordinates 
    ...
    pick(params) { // Returns info on the object at the given canvas coordinates
        ...
        //console.log("pick_uniqueId",uniqueId);
        if (!canvasPos&&!uniqueId) {
        ...
        console.log("Picked__@", this.tmp_unproject[0], this.tmp_unproject[1], this.tmp_unproject[2], uniqueId, viewObject);
        ...
        return { object: null, coordinates: this.tmp_unproject, depth: depth };
            ...
            var viewObject =this.getViewObject(uniqueId); 
            if (viewObject) {
                var uniqueId = viewObject.uniqueId;
                if (params.select !== false) {
                    if (!params.shiftKey) {
                ...
                return { object: viewObject, normal: null, coordinates: this.tmp_unproject, depth: null };
            } else if (params.select !== false) {
                if (this.selectedElements.size > 0) {
                    this.eventHandler.fire("selection_state_changed", this.selectedElements, false);
                    this.selectedElements.clear();
