...
export class BimSurfer extends EventHandler {
	...
	load(params) {
		return new Promise((resolve, reject) => {
			...
			this.loadProjects(params.roid).then((project)=>{                
				...
				console.log("view",this._bimServerViewer);
			}).catch(reject);
		});
	}
	...
	setVisibility(params) {
		...
		if(params==null||params==""){
			return v.setVisibility(v.selectedElements, false,false);
		}
		if (params.ids) {
	...
	setCameraQuick(params) {
		let v = this._bimServerViewer.viewer;
		v.changeCameraEye(params);
	}
	...
	screenshot(select){
		this._bimServerViewer.viewer.download(select);
	}

	getStats(){
		console.log("GPU bytes",this._bimServerViewer.stats.get("Data","GPU bytes"));
		console.log("GPU bytes total",this._bimServerViewer.stats.get("Data","GPU bytes total"));
		console.log("Data",this._bimServerViewer.stats.get("Data","GPU bytes"));
