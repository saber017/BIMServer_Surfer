...
export class RenderLayer {
	...
	createObject(loaderId, roid, uniqueId, geometryIds, matrix, normalMatrix, scaleMatrix, hasTransparency, type, aabb, gpuBufferManager, node) {
		//console.log("renderLayercreateObject",loaderId, roid, uniqueId, geometryIds, matrix, normalMatrix, scaleMatrix, hasTransparency, type, aabb, gpuBufferManager, node);
		var loader = this.getLoader(loaderId);
		var object = {
			uniqueId: uniqueId,
			hasTransparency: hasTransparency,
...
//				object: this.viewer.model.objects[oid],
			add: (geometryId, uniqueId) => {//添加几何信息
		...
		//console.log("createObject,viewObject",viewObject);
	...
	addGeometry(loaderId, geometry, object, buffer, sizes) {
		console.log("addGeometry",loaderId, geometry, object, buffer, sizes);
	...
	render(transparency, lineRender, visibleElements) {
		//console.info("render455"+Object.keys(visibleElements));
