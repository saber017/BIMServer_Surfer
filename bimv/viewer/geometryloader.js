...
export class GeometryLoader {
	constructor(loaderId, renderLayer, loaderSettings, vertexQuantizationMatrices, stats, settings, geometryCache, gpuBufferManager, usePreparedBuffers) {
		...
		if (usePreparedBuffers) {
			this.createdTransparentObjects = new Map(); // object id -> object info透明
			this.createdOpaqueObjects = new Map(); // object id -> object info不透明
		}
...
//console.log("this.preparedBuffer.pickColors",this.preparedBuffer.pickColors);
	...
	processPreparedBuffer(stream, hasTransparancy) {
		...
		//console.log("this.preparedBuffer",this.preparedBuffer)
		const loadedViewObjects = [];

		const nrObjects = stream.readInt();//数量
		const totalNrIndices = stream.readInt();//所有索引
		const totalNrLineIndices = stream.readInt();//所有线索引
		const positionsIndex = stream.readInt();//位置索引
		const normalsIndex = stream.readInt();//法线索引
		const colorsIndex = stream.readInt();//颜色索引
		//console.log("processPreparedBuffer106",nrObjects,totalNrIndices,totalNrLineIndices,positionsIndex,normalsIndex,colorsIndex);
		...
		pickColors.i = 0;
		//console.log("processPreparedBuffer106",pickColors,pickColors32)
		...
		for (var i = 0; i < nrObjects; i++) {
			...
			var nrVertices = stream.readInt();//顶点
			...
			//console.log("processPreparedBuffer","viewObject",viewObject,"pickColor",pickColor,color32,pickColors.i,nrObjectColors);
			...
			// if (colorPackSize != 0){
			// 	console.log("colorPackSize",colorPackSize);
			// }
			//console.log("colorPackSize",colorPackSize);
			if (colorPackSize == 0) {
...
//			}
	...
	readObject(stream, geometryType) {
		//console.log("readObject",geometryType);
		var geometryId;
		var numGeometries;
		var numParts;
		var objectBounds;
	...
	readGeometry(stream, roid, croid, geometryId, geometryDataOid, hasTransparency, reused, type, useIntForIndices) {
		//console.log("readGeometry",roid, croid, geometryId, geometryDataOid, hasTransparency, reused, type, useIntForIndices);
	...
	createObject(roid, uniqueId, geometryIds, matrix, hasTransparency, type, aabb, inCompleteBuffer) {
		//console.log("createObject",roid, uniqueId, geometryIds, matrix, hasTransparency, type, aabb, inCompleteBuffer);
