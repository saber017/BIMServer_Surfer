    ...
    function MetaDataRenderer(args) {
        ...
        this.setSelected = function(oid) {
            ...
            if (oid.indexOf(':') !== -1) {
                ...
                var uniqueId=Number(oid[1]);
                var viewObject= window.bimSurfer._bimServerViewer.viewer.getViewObject(uniqueId);
                //window.bimSurfer._bimServerViewer.viewer.pick({uniqueId:uniqueId});
                //window.bimSurfer._bimServerViewer.viewer.dirty=2;
                var selected=new Array();
                selected.push(uniqueId);
                if (selected.length) {
                    window.bimSurfer.viewFit({
                        ids: selected,
                        animate: true
                    });
                }
                window.bimSurfer.setSelectionState({
                    ids: selected,
                    clear: true,
                    selected: true
                });
                console.log("viewObjects",viewObject,oid[1]);
                //window.bimsurfer.setSelected(oid[1]);
        ...
        this.beSelected = function(oid) {
            console.log("调用成功",oid);
            ...
            if (oid.indexOf(':') !== -1) {
                ...
                // var uniqueId=Number(oid[1]);
                // var viewObject= window.bimSurfer._bimServerViewer.viewer.getViewObject(uniqueId);
                // window.bimSurfer._bimServerViewer.viewer.invisibleElements.add(uniqueId);
                // window.bimSurfer._bimServerViewer.viewer.dirty=2;
                //window.bimsurfer.setSelected(oid[1]);
                var o = models[oid[0]].apiModel.objects[oid[1]];
                renderAttributes(o);
